...
{
	// process POST data
	if (check_bitrix_sessid() && (!empty($_REQUEST["{$arParams['PREFIX_FORM']}_iblock_submit"]) || !empty($_REQUEST["{$arParams['PREFIX_FORM']}_iblock_apply"])) && empty($_POST["ROBOT_TEST"]))
	{
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			$_SESSION["CONTACTS_ADDED_MESSAGE_{$arParams['PREFIX_FORM']}"] = $arParams["USER_MESSAGE_".$sAction];
	...
	if ($bShowForm)
	{
		...
		$arResult["MESSAGE"] = $_SESSION["CONTACTS_ADDED_MESSAGE_{$arParams['PREFIX_FORM']}"];
        unset($_SESSION["CONTACTS_ADDED_MESSAGE_{$arParams['PREFIX_FORM']}"]);
