$(document).ready(function() {
	...
	sequence.afterLoaded = function(){
		...
		setTimeout(function(){
			$("#sequence h2").fitText(0.9, { minFontSize: '24px', maxFontSize: '36px' });
			$("#sequence p").fitText(1.2, { minFontSize: '12px', maxFontSize: '24px' });
		},500);
	};
	...
	$('#ajax-load').click(function(e){
		...
		var moreLink = 'page.php?PAGEN_1='+moreSet;
        
        $.get(moreLink, function(data){
			
			$('#ajax-load').html('<i class="fa fa-plus-circle"></i>Показать еще');
		...
		if (moreSet>portfolio_count_page){
			$(this).hide();
		}
    });
